...
```{r eval=FALSE}
survey_design %>%
  summarise(median_income = survey_median(Expenditure,
                                    level = 0.95,
                                    vartype =  c("se", "ci")
                                    )
            )
...
```{r, eval=TRUE}
region_mean <- svyby(formula = ~Income,
                     by = ~Region,
...
```{r}
res <- svycontrast(stat = region_mean,
...
```{r}
res <- svycontrast(stat = sex_mean,
                   contrasts = list(sex_diff = c(1, -1)))
```
...
sex_ratio <- svyby( formula = ~Income,
                     by = ~Sex,
...
```{r}
res <- svycontrast(stat = sex_ratio,
                   contrasts = list(
                          sex_diff = c(1, -1))
                   )
